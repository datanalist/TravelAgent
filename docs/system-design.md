...
- **Отдельный микросервис на каждый канал** — дублирование всей бизнес-логики (Decision Logic, Memory Layer, Profile Updater) в каждом сервисе. Синхронизация состояния между сервисами потребовала бы message bus (Kafka/RabbitMQ), что избыточно для MVP.
- **BFF (Backend-for-Frontend) паттерн** — отдельный слой агрегации под Web и под Telegram. Добавляет hop в latency и усложняет деплой без ощутимой выгоды при двух каналах.
...
**Отклонённые альтернативы:**
- **LLM-оракул с RAG (поиск туров в embedding-пространстве)** — LLM сам «знает» туры через RAG и отвечает напрямую. Отклонено: RAG может пропустить релевантный тур или вернуть устаревшие данные. Цены и наличие мест меняются ежедневно; RAG-индекс всегда лагает. Детерминированный `search_tours` гарантирует актуальность данных.
- **Fine-tuned модель на каталоге туров** — специализированная модель, обученная на базе туров. Отклонено: дорого в поддержке, требует переобучения при изменении каталога, нет гарантий против галлюцинаций цен.
...
**Отклонённые альтернативы:**
- **Только Redis** — быстро, но нет персистентности между сессиями. Перезапуск Redis или истечение TTL = потеря всей истории клиента. Неприемлемо для профиля (предпочтения, история покупок).
- **Только PostgreSQL** — надёжно, но latency 10–50ms на каждый read/write в hot-path. При 5–10 шагах агента на сообщение суммарный overhead составил бы 50–500ms дополнительно к LLM-latency.
- **MongoDB / документная БД** — гибкая схема JSONB. Отклонено: PostgreSQL уже поддерживает JSONB-поля с нужной гибкостью, без добавления нового компонента инфраструктуры.
...
- **LangGraph (граф агентов)** — популярный фреймворк для оркестрации агентов на базе направленного графа состояний. Отклонено в MVP по нескольким причинам: (1) vendor lock-in на LangChain-экосистему с собственными абстракциями поверх LLM API; (2) трудная отладка — граф скрывает поток данных и делает стектрейсы нечитаемыми; (3) overhead абстракций добавляет latency и усложняет кастомизацию; (4) наш Decision Logic умещается в ~100 строк Python, LangGraph здесь избыточен. _Возможно рассмотреть_ при масштабировании до 5+ типов агентов.

- **Полный LLM-router** — LLM самостоятельно решает следующий шаг без rule-based ограничений, получает все tools сразу. Отклонено: (1) недетерминированное поведение — в продакшне сложно воспроизвести баг; (2) LLM может вызвать `create_lead` или `update_client_profile` преждевременно (Risk R8 — Tool Injection); (3) дополнительный LLM-вызов на каждое решение = +$0.003–0.01 и +500–1500ms к latency.

- **Жёсткая State Machine (FSM) как единственный контроллер** — строгая FSM определяет не только стадию, но и конкретный следующий вопрос и действие агента. Отклонено: (1) FSM не обрабатывает свободный NL-ввод без комбинаторного взрыва переходов; (2) гибкость high-end диалога (тон, контекст, нюансы) невозможна в жёстких правилах; (3) поддержка требует постоянного ручного расширения переходов. Принятое решение: FSM управляет _стадией_ (stage), LLM управляет _текстом_ — разделение ответственностей.
...
**Отклонённые альтернативы:**
- **WebSockets** — двунаправленный протокол. Отклонено: в нашем случае направление всегда server→client (токены ответа), WebSocket избыточен и сложнее в деплое (sticky sessions, timeouts). SSE работает поверх обычного HTTP/2.
- **Long polling** — клиент периодически запрашивает новые токены. Отклонено: высокая нагрузка на backend (N запросов на одно сообщение), плохой UX при задержках (заметные паузы).
...
**Отклонённые альтернативы:**
- **LiteLLM** — готовая библиотека-абстракция над 100+ LLM-провайдерами. Рассматривалась всерьёз: хорошая совместимость, активно поддерживается. Отклонено для MVP: добавляет зависимость с собственным поведением при ошибках, ограничивает кастомизацию retry/circuit-breaker логики, скрывает детали streaming. Можно вернуться при расширении числа провайдеров > 3.
- **Жёсткая привязка к одному провайдеру** — проще в реализации. Отклонено: при деградации провайдера (outage Anthropic) вся система падает. Абстракция позволяет переключиться за изменение одной env-переменной.
...
**Отклонённые альтернативы:**
- **Без лимита шагов** — агент работает до получения финального ответа. Отклонено: один запрос может вызвать 20+ tool-calls при плохом промпте или prompt injection, стоимость и latency становятся непредсказуемыми.
- **Лимит 3 шага** — более консервативно. Отклонено: типовой сценарий (поиск туров + уточнение + создание лида) требует 3 tool-calls, плюс возможные уточнения. 3 шага создаёт ограничения на реальных сценариях. 5 — разумный баланс.
...
**Когда запускается:** При превышении порога длины истории (> 10 сообщений без пересчёта summary или > 12 000 токенов в контексте). Не вызывается после каждого сообщения, чтобы не удваивать расход LLM-токенов.

**Алгоритм:**
1. Берёт последние N сообщений из `messages` (PostgreSQL).
2. Вызывает отдельный LLM-prompt (см. ниже).
3. Записывает summary в `session:{id}:summary` (Redis) и `sessions.summary` (PostgreSQL).

#### Промпт Conversation Summarizer

**System prompt:**
```
Ты — аналитик диалогов для туристического агентства. Твоя задача: сжать историю диалога менеджера с клиентом в компактное структурированное резюме.

Правила:
- Извлеки только факты, явно выраженные клиентом. Не домысливай и не интерпретируй.
- Максимум 5 фактов в client_facts. Приоритет: бюджет > направление > даты > стиль > прочее.
- current_request — одна фраза, описывающая актуальный запрос клиента прямо сейчас.
- stage — текущая стадия из списка: cold, discovery, qualified, proposal, objection, closing, follow_up.
- last_shown_tours — только ID туров, которые агент уже предложил клиенту (из tool_results).
- Если факт неизвестен — не включай его в список.
- Отвечай строго в JSON-формате, без пояснений.
```

**User prompt (шаблон):**
```
Сожми следующий диалог в структурированное резюме.
...
Верни JSON строго следующей структуры:
{
  ...
  "updated_at": "<ISO8601 текущего момента>"
}
```
...
|---|---|---|
| `max_tokens` | 300 | Summary должно быть коротким |
| `temperature` | 0.0 | Детерминированный вывод, нет нужды в креативности |
| `provider` | Дешёвая модель (Mistral / GPT-4o-mini) | Снизить стоимость: summary — не основной ответ |
| `response_format` | `{ "type": "json_object" }` | Принудительный JSON |

**Пример входа:**
```
[user]: Хочу слетать куда-нибудь в тепло в феврале
[assistant]: Отличный выбор! Расскажите, пожалуйста, о вашем бюджете и откуда планируете вылетать?
[user]: Из Москвы, бюджет тысяч 150-200 на двоих, хотим 5*
[assistant]: Понял. Рассматриваете конкретные направления — Мальдивы, ОАЭ, Таиланд?
[user]: Мальдивы или ОАЭ, желательно с системой всё включено
[assistant]: [показаны туры тур_abc и тур_def]
```
...
{
  "client_facts": [
    "бюджет 150–200 тыс. руб. на двоих",
    "вылет из Москвы",
    "февраль",
    "предпочитает 5* отели",
    "All Inclusive обязательно"
  ],
  "current_request": "подбор тура Мальдивы или ОАЭ в феврале",
  "stage": "proposal",
  "last_shown_tours": ["тур_abc", "тур_def"],
  "updated_at": "2026-02-01T14:30:00Z"
}
```

**Валидация вывода:** Orchestrator проверяет JSON-схему. При ошибке парсинга — повторный вызов (max 2 retry), затем fallback: сохранить старый summary без обновления.
